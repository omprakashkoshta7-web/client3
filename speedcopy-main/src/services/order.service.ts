...
// Add remaining interfaces and class definition below...
 ...
 */
class OrderService {
  private normalizeLocalCart(value: any) {
  ...
  private parseLocalArray(key: string) {
    try {
  ...
  // ============================================
  // IndexedDB Helper Methods (for large files)
  // ============================================
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains('preview_cache')) {
          db.createObjectStore('preview_cache', { keyPath: 'id' });
        }
      };
    });
  }

  private async storePreviewInIndexedDB(customizationId: string, base64Preview: string): Promise<boolean> {
    try {
      const db = await this.openIndexedDB();
      if (!db) return false;
      return new Promise((resolve, reject) => {
        const tx = db.transaction('preview_cache', 'readwrite');
        const store = tx.objectStore('preview_cache');
        const request = store.put({ id: customizationId, preview: base64Preview, timestamp: Date.now() });
        request.onerror = () => reject(request.error);
        request.onsuccess = () => resolve(true);
      });
    } catch (err) {
      console.warn('Failed to store preview in IndexedDB:', err);
      return false;
    }
  }
  ...
  async getCart() {
    try {
      ...
      try {
        return wrapSuccess(this.normalizeLocalCart(localCart ? JSON.parse(localCart) : null));
      } catch {
  ...
  async addToCart(data: {
    ...
    try {
      ...
      // Store large base64 preview in IndexedDB BEFORE sending to API
      const customizationId = data.customization?.customizationId || data.designId || `cart_${Date.now()}`;
      if (data.designPreview && data.designPreview.startsWith('data:image')) {
        await this.storePreviewInIndexedDB(customizationId, data.designPreview);
        console.log('✅ Preview cached in IndexedDB');
      }
      
      // Create API payload WITHOUT large base64 to avoid 413 error
      const apiPayload = { ...data };
      if (apiPayload.designPreview?.startsWith('data:image')) {
        delete apiPayload.designPreview; // Remove base64 before sending to API
      }
      
      const response = await apiClient.post('/api/orders/cart', apiPayload);
      console.log('✅ Added to cart:', response.data);
      return response.data;
    } catch (error) {
      console.warn('⚠️ Add to cart API failed:', error);
      
      // Fallback to localStorage with IndexedDB support
      const localCart = localStorage.getItem('speedcopy_cart');
      let cart = this.normalizeLocalCart(null);
      try {
        cart = this.normalizeLocalCart(localCart ? JSON.parse(localCart) : null);
      } catch {
        localStorage.removeItem('speedcopy_cart');
      }
      
      // Store large preview in IndexedDB
      const customizationId = data.customization?.customizationId || data.designId || `cart_${Date.now()}`;
      if (data.designPreview && data.designPreview.startsWith('data:image')) {
        try {
          await this.storePreviewInIndexedDB(customizationId, data.designPreview);
          console.log('✅ Preview stored in IndexedDB for offline mode');
        } catch (idbErr) {
          console.warn('Failed to cache in IndexedDB:', idbErr);
        }
      }
      
      // Store cart item WITHOUT the large base64
      const cartItem = { ...data, _id: `cart_${Date.now()}` };
      if (cartItem.designPreview?.startsWith('data:image')) {
        delete cartItem.designPreview; // Remove base64 to keep cart small
      }
      
      cart.items.push(cartItem);
      localStorage.setItem('speedcopy_cart', JSON.stringify(cart));
      
      return wrapSuccess(cart, 'Added to cart (offline mode)');
    }
  }
  ...
  async getMyOrders(params?: { 
    ...
    try {
      ...
      // Fallback to localStorage
      const orders = this.parseLocalArray('speedcopy_orders');
  ...
  async getOrderSummary(): Promise<{ success: boolean; data: OrderSummary }> {
    try {
      ...
      // Fallback to localStorage
      const orders: Order[] = this.parseLocalArray('speedcopy_orders');
  ...
  async trackOrder(orderId: string): Promise<{ success: boolean; data: TrackingView }> {
    try {
      ...
      return wrapSuccess(trackingView);
    }
  }
  ...
  async getEditWindow(orderId: string): Promise<{ success: boolean; data: OrderEditWindow }> {
    try {
      ...
      const orderResponse = await this.getOrderById(orderId);
      return wrapSuccess(orderResponse.data.editWindow);
    }
  }
