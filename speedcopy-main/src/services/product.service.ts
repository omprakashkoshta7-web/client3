...
export interface ProductVariant {
  ...
  previewImages?: Array<{ url?: string; type?: string; label?: string }>;
  media?: Array<{ url?: string; type?: string; label?: string }>;
}
...
class ProductService {
  private normalizeVariant(variant: ProductVariant): ProductVariant {
    const previewImages = Array.isArray(variant.previewImages)
      ? variant.previewImages
          .map((img) => ({ ...img, url: normalizeImageUrl(img?.url) }))
          .filter((img) => Boolean(img.url))
      : [];
    const mediaImages = Array.isArray(variant.media)
      ? variant.media
          .map((img) => ({ ...img, url: normalizeImageUrl(img?.url) }))
          .filter((img) => Boolean(img.url))
      : [];
    const previewThumbnail =
      previewImages.find((img) => img.type === 'thumbnail')?.url ||
      mediaImages.find((img) => img.type === 'thumbnail')?.url ||
      previewImages[0]?.url ||
      mediaImages[0]?.url ||
      '';
    const directThumbnail = normalizeImageUrl(
      variant.thumbnail ||
    ...
    return {
      ...variant,
      size: variant.size || attributes.size,
      size_label: variant.size_label || attributes.size_label || attributes.size,
      paper_type: variant.paper_type || attributes.paper_type || attributes.paper,
      cover_color_name: variant.cover_color_name || attributes.color || attributes.cover_color_name,
      additional_price: variant.additional_price ?? variant.pricing?.salePrice ?? variant.price ?? 0,
      images: Array.isArray(variant.images) ? variant.images.map(normalizeImageUrl).filter(Boolean) : variant.images,
      thumbnail: directThumbnail || previewThumbnail,
      imageUrl: directThumbnail || previewThumbnail,
      image: directThumbnail || previewThumbnail,
      previewImages,
      media: mediaImages,
    };
  }
  ...
  private normalizeProduct(product: Product): Product {
    return {
      ...product,
      images: Array.isArray(product.images) ? product.images.map(normalizeImageUrl).filter(Boolean) : product.images,
  ...
  private normalizeProductsResponse(response: ApiResponse<ProductsResponse>): ApiResponse<ProductsResponse> {
    return {
      ...response,
      data: {
        ...response.data,
        products: (response.data?.products || []).map(p => this.normalizeProduct(p)),
      },
    };
  }
  ...
  async getProducts(query: ProductListQuery = {}): Promise<ApiResponse<ProductsResponse>> {
    try {
      ...
      const normalized = this.normalizeProductsResponse(response.data);
  ...
  async getProductById(id: string): Promise<ApiResponse<Product>> {
    try {
      ...
      // Cache product
      localStorage.setItem(`product_${id}`, JSON.stringify(normalized));
  ...
  async getProductBySlug(slug: string): Promise<ApiResponse<Product>> {
    try {
      ...
      // Cache product
      localStorage.setItem(`product_slug_${slug}`, JSON.stringify(normalized));
   ...
   * GET /api/products/variants?productId=:id
   */
  async getVariantsByProduct(productId: string): Promise<ProductVariant[]> {
    try {
      const response = await apiClient.get(
        API_CONFIG.ENDPOINTS.PRODUCTS.GENERAL.VARIANTS_BY_PRODUCT(productId)
      );
      ...
      console.warn('⚠️ getVariantsByProduct failed:', error);
      return [];
    }
  }
  ...
  async getGiftingProductById(id: string): Promise<ApiResponse<Product>> {
    try {
      ...
      // Cache product
      localStorage.setItem(`gifting_product_${id}`, JSON.stringify(normalized));
  ...
  async getShoppingProductById(id: string): Promise<ApiResponse<Product>> {
    try {
      ...
      // Cache product
      localStorage.setItem(`shopping_product_${id}`, JSON.stringify(normalized));
  ...
  /**
   * Get business printing product by ID
   */
  async getBusinessProductById(id: string): Promise<ApiResponse<Product>> {
    try {
      if (!this.isValidObjectId(id)) {
        throw new Error(`Invalid product ID format: ${id}`);
      }

      console.log('🚀 [Product Service] Fetching business printing product by ID:', id);
      const response = await apiClient.get<ApiResponse<Product>>(
        API_CONFIG.ENDPOINTS.PRODUCTS.BUSINESS_PRINTING.PRODUCT_BY_ID(id)
      );
      console.log('✅ [Product Service] Business printing product fetched:', response.data);
      const normalized = {
        ...response.data,
        data: this.normalizeProduct(response.data.data),
      };
      ...
      // Cache product
      localStorage.setItem(`business_product_${id}`, JSON.stringify(normalized));
      
      return normalized;
    } catch (error: any) {
      console.error('❌ [Product Service] Failed to fetch business product by ID:', error);
      
      // Fallback to localStorage
      const cachedData = localStorage.getItem(`business_product_${id}`);
      if (cachedData) {
        console.log('⚠️ [Product Service] Using cached business product');
        return JSON.parse(cachedData);
      }
