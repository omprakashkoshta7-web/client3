...
const GiftingCheckoutPage: React.FC = () => {
  ...
  const fetchCheckoutData = async () => {
    try {
      ...
      if (isAuthenticated) {
        ...
        const allItems = normalizeCartItems(rawCart);
        
        // Filter by gifting flow type
        const filteredItems = allItems.filter((item: any) => item.flowType === 'gifting');
        
        console.log('🛒 [Gifting Checkout] Cart Data:');
        console.log('   Total items from cart:', allItems.length);
        console.log('   Filtered gifting items:', filteredItems.length);
        console.log('   Items:', filteredItems.map((item: any) => ({
        ...
        // Calculate subtotal from filtered items - normalize quantity field
        const filteredSubtotal = filteredItems.reduce(
          (sum: number, item: any) => {
            const price = item.unitPrice || item.price || 0;
            const qty = item.quantity || item.qty || 1;
            const itemTotal = price * qty;
            console.log(`   Item: ${item.productName} - ${price} × ${qty} = ${itemTotal}`);
            return sum + itemTotal;
          }, 0
        );
        
        console.log('   ✅ Filtered Subtotal:', filteredSubtotal);
  ...
  const handlePayment = async () => {
    try {
      setProcessing(true);
      const cartItems = normalizeCartItems(orderSummary);
      ...
      // Normalize quantity and price fields - handle both 'quantity' and 'qty' properties
                  ...
                  return (
                    <div key={i} className="flex items-center justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-xs" style={{ color: '#9ca3af' }}>Qty: {qty}</p>
                      </div>
