...
const ProductDetailPage: React.FC = () => {
      ...
      const response = requestedFlow === 'gifting'
        ...
        : requestedFlow === 'shopping'
          ...
          : searchParams.get('type') === 'business'
            ? await productService.getBusinessProductById(id!).catch(() => productService.getProductById(id!))
  ...
  const normalizedImages = useMemo(() => {
    ...
    console.log('📸 [ProductDetail] Raw image values:', vals);
    const normalized = vals.length ? normalizeImageList(vals) : [];
    console.log('📸 [ProductDetail] Normalized images:', normalized);
  ...
  const handleImageError = (e: React.SyntheticEvent<HTMLImageElement>, originalSrc: string) => {
    ...
    if (!originalSrc.includes('unsplash') && originalSrc.includes('cdn-202671058278')) {
      // Already on CDN, try uploads URL
      const uploadsUrl = originalSrc.replace('cdn-202671058278', 'uploads-202671058278');
      console.log('🔄 Retrying with uploads CDN:', uploadsUrl);
      e.currentTarget.src = uploadsUrl;
    } else if (!originalSrc.includes('unsplash') && !originalSrc.includes('cdn-')) {
      // Use CDN fallback
      console.log('🔄 Retrying with CDN fallback');
      const UPLOADS_BASE_URL = 'https://uploads-202671058278.asia-east1.run.app';
      const path = originalSrc.match(/\/uploads\/.+/)?.[0] || '';
      if (path) {
        const cdnUrl = `${UPLOADS_BASE_URL}${path}`;
        e.currentTarget.src = cdnUrl;
      }
    } else {
      // Use placeholder
      e.currentTarget.src = 'https://images.unsplash.com/photo-1514228742587-6b1558fcca3d?w=900&q=80';
    }
  };
  ...
  // Calculate real delivery date based on current time + business days
  const calcDeliveryDate = (businessDays: number): string => {
    const now = new Date();
    const cutoffHour = 14; // 2 PM cutoff — orders after 2 PM count as next day
    
    // If order placed after cutoff, start counting from tomorrow
    ...
    while (daysAdded < businessDays) {
      ...
      if (day !== 0) { // skip Sundays (India — Saturdays are working)
        daysAdded++;
      }
    }
  ...
  const checkPincode = async () => {
    ...
    try {
      ...
      if (data?.[0]?.Status === 'Success' && data[0].PostOffice?.length > 0) {
        ...
        // Determine delivery speed based on state
        // Same state = 2–3 days, different state = 4–5 days
        // We'll use a simple heuristic: metro pincodes get faster delivery
        const metroRanges = [
          [110001, 110099], // Delhi
          ...
          [380001, 380099], // Ahmedabad
          [411001, 411099], // Pune
        ];
        const pin = parseInt(pincodeInput);
        const isMetro = metroRanges.some(([min, max]) => pin >= min && pin <= max);
        
        const minDays = isMetro ? 2 : 3;
        const maxDays = isMetro ? 3 : 5;
        const minDate = calcDeliveryDate(minDays);
        const maxDate = calcDeliveryDate(maxDays);
        
        const now = new Date();
        const cutoffHour = 14;
        const orderNote = now.getHours() >= cutoffHour
          ? 'Order now to dispatch tomorrow'
          : 'Order now to dispatch today';

        setPincodeInfo({
          city: po.District || po.Name,
          state: po.State,
          estimatedDays: `${minDate} – ${maxDate}`,
          orderNote,
          minDays,
          maxDays,
        } as any);
  ...
  const addToCart = async (redirectTo?: string) => {
    ...
    // Require frame selection only when frames are available
    if (isPrintingProduct && hasFrames && !selectedFrame) {
      setCartError('Please select a frame first');
    ...
    try {
      ...
      const existingItems = cartResponse?.data?.items || cartResponse?.items || [];
      const productId = product?._id || product?.id || id;
      const variantId = selectedVariant?.id || String(selectedVariant?.index ?? selectedVariantIndex);
      
      const existingItem = existingItems.find((item: any) => 
        String(item.productId || item.product_id) === String(productId) && 
        String(item.variantId || item.variant_id) === String(variantId)
      );
      
      if (existingItem) {
        setDuplicateItemModal({ 
          show: true, 
          existingQty: existingItem.quantity || existingItem.qty || 1,
  ...
  return (
    <div style={{ backgroundColor: '#f5f5f5', minHeight: '100vh' }}>
        ...
        <div className="grid lg:grid-cols-[1fr_1fr] gap-12 mb-14">
          ...
          <div>
            ...
            >
              <img
                ...
                alt={productName}
                onError={(e) => handleImageError(e, normalizedImages[Math.min(activeImg, normalizedImages.length - 1)])}
          ...
          <div className="pt-2">
            ...
            {product?._id && (
              <div className="mb-6">
                <ProductFramesSelector
                  ...
                  onFramesAvailable={(available) => setHasFrames(available)}
                />
              </div>
            )}
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              {cartError && (
                <div className="text-xs text-red-500 text-center mb-2 bg-red-50 py-1.5 px-3 rounded-lg font-medium">
                  {cartError}
                </div>
              )}
              {/* Add to cart */}
            ...
            <div className="mt-4 rounded-2xl bg-white p-4" style={{ border: '1px solid #e5e7eb' }}>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onKeyDown={e => e.key === 'Enter' && pincodeInput.length === 6 && checkPincode()}
                  className="flex-1 px-3 py-2 rounded-xl text-sm text-gray-900 outline-none"
                  style={{ border: '1.5px solid #e5e7eb', backgroundColor: '#fafafa' }}
                />
                <button
                  onClick={checkPincode}
                  disabled={pincodeInput.length !== 6 || pincodeStatus === 'checking'}
              ...
              {pincodeStatus === 'available' && pincodeInfo && (
                <div className="mt-3 p-3 rounded-xl" style={{ backgroundColor: '#f0fdf4', border: '1px solid #bbf7d0' }}>
                  <div className="flex items-start gap-2 mb-2">
                    <svg className="w-4 h-4 mt-0.5 flex-shrink-0" style={{ color: '#16a34a' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                    </svg>
                    <p className="text-xs font-bold" style={{ color: '#166534' }}>
                      Delivery available to {(pincodeInfo as any).city}, {(pincodeInfo as any).state}
                    </p>
                  </div>
                  {/* Delivery timeline */}
                  <div className="ml-6 space-y-1.5 mb-3">
                    <div className="flex items-center gap-2">
                      <svg className="w-3.5 h-3.5 flex-shrink-0" style={{ color: '#15803d' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                      </svg>
                      <p className="text-xs font-semibold" style={{ color: '#15803d' }}>
                        Expected by: {(pincodeInfo as any).estimatedDays}
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      <svg className="w-3.5 h-3.5 flex-shrink-0" style={{ color: '#15803d' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      ...
                      <p className="text-xs" style={{ color: '#15803d' }}>
                        {(pincodeInfo as any).orderNote} · Cutoff: 2:00 PM
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      <svg className="w-3.5 h-3.5 flex-shrink-0" style={{ color: '#15803d' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      ...
                      <p className="text-xs" style={{ color: '#15803d' }}>
                        {(pincodeInfo as any).minDays}–{(pincodeInfo as any).maxDays} business days · Sundays excluded
                      </p>
                    </div>
                  </div>
                  {/* Use this pincode button */}
                  <button
                    onClick={() => navigate('/checkout', {
                      state: {
                        flow: flowType,
                        deliveryPincode: pincodeInput,
                        deliveryCity: (pincodeInfo as any).city,
                        deliveryState: (pincodeInfo as any).state,
                        estimatedDelivery: (pincodeInfo as any).estimatedDays,
                        orderNote: (pincodeInfo as any).orderNote,
                      }
                    })}
                    className="w-full py-2 rounded-xl text-xs font-bold text-white transition"
                    style={{ backgroundColor: '#111' }}
                  >
                    Use this pincode for delivery →
                  </button>
                </div>
              )}

              {pincodeStatus === 'unavailable' && (
                <div className="mt-3 flex items-start gap-2 p-3 rounded-xl" style={{ backgroundColor: '#fef2f2', border: '1px solid #fecaca' }}>
                  <svg className="w-4 h-4 mt-0.5 flex-shrink-0" style={{ color: '#dc2626' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                  </svg>
                  <div>
                    <p className="text-xs font-bold" style={{ color: '#991b1b' }}>Delivery not available for this pincode</p>
                    <p className="text-xs mt-0.5" style={{ color: '#b91c1c' }}>Try a nearby pincode or choose pickup.</p>
                  </div>
                </div>
              )}
            </div>
      ...
      {duplicateItemModal.show && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/50" onClick={() => setDuplicateItemModal({ show: false, existingQty: 0, itemName: '' })} />
          <div className="relative bg-white rounded-2xl p-6 w-full max-w-sm text-center shadow-xl">
            ...
            <h3 className="text-lg font-bold text-gray-900 mb-2">Item Already in Cart!</h3>
            <p className="text-sm text-gray-600 mb-1">
              <span className="font-semibold">{duplicateItemModal.itemName}</span> is already in your cart ({duplicateItemModal.existingQty} qty).
            </p>
            <p className="text-sm text-gray-600 mb-4">Would you like to add more or view your cart?</p>
            <div className="flex gap-3">
              <button
                onClick={() => setDuplicateItemModal({ show: false, existingQty: 0, itemName: '' })}
              ...
              <button
                onClick={() => {
                  setDuplicateItemModal({ show: false, existingQty: 0, itemName: '' });
                  navigate('/cart');
                }}
